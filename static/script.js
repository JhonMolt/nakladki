...
if(aboutText){const words=aboutText.querySelectorAll('.about-word');const aboutSection=document.querySelector('.about');window.addEventListener('scroll',()=>{const rect=aboutSection.getBoundingClientRect();const sectionHeight=rect.height;const vh=window.innerHeight;const scrollProgress=Math.max(0,Math.min(1,(vh*.5-rect.top)/(vh*.4+sectionHeight*.4)));words.forEach((word,i)=>{const wordProgress=scrollProgress*words.length-i;if(wordProgress>0){word.classList.add('visible')}else{word.classList.remove('visible')}})})}
